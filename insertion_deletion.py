...
def insert_regular_markers(m, Nc, marker_sequence, perm):
    """
    Insert marker sequence into the message bits, regularly. I.e., a pre-selected marker sequence with Nm
    ...
    - perm (numpy.ndarray): Interleaving
    ...
    # Get parameters of the code from the specified marker sequence and Nc.
    ...
    # If message length does not divide Nc, then pad minimum number of random message bits 
    # until message length divides Nc.
    if m.shape[-1] % Nc != 0:
        m = np.concatenate((m, np.random.randint(low=0, high=2, size=(1, Nc-(m.shape[-1] % Nc)), dtype=int)), axis = 1) 
